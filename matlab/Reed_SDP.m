...
if nargin < 2; DISCRETISE = 100; SIM = 1; disp('Running defaults'); end
A = 2; B = 4; % Beverton-Holt parameters
...
S = length(n_vec); d = n_vec(2)-n_vec(1); % Number of states and discretisation
HVec = n_vec; L_H = length(HVec); SDP_Mat = zeros(S,S,L_H);
% CREATE TRANSITION MATRICES CORRESPONDING TO ALTERNATE ACTIONS
for q = 1:L_H % Cycle through all the harvest options
    ...
    for i = 1:S 
        ...
        if x2_exp == 0; SDP_Mat(i,1,q) = 1; else % If all transitions are to negative abundances
            ...
            SDP_Mat(i,:,q) = Prob./sum(Prob); % Store the normalised transition probability in the transition matrix
        end
    end
end
...
OptTime = 25; for Timestep = 1:OptTime
    for i = 1:L_H % Try out all the potential harvest rates
    %eek V1 not initialized!

        
        V1(:,i) = SDP_Mat(:,:,i)*V + min(HVec(i),n_vec') * exp(-delta*(OptTime-Timestep)); % Consider the ongoing reward
    end
%% What's going on with these weird arguments to max?
