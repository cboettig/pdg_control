...
<<settings, echo=FALSE, warning=FALSE>>=
require(knitr)
opts_chunk$set(echo=FALSE, dev="pdf", cache=FALSE, warning=FALSE)
require(pdgControl)
require(reshape2)
require(ggplot2)
require(data.table)
...
<<maincode, echo=FALSE, warning=FALSE>>=
#source("writeup.R")
load("writeup.rda")
@
<<errortable, echo=FALSE, warning=FALSE>>=
#source("errors_table.R")
NULL
@
...
\begin{figure}
<<Figure_1, cache=FALSE>>=
ggplot(fees, aes(c2, (npv0-value)/npv0, col=variable)) + geom_point() + geom_line()+theme_bw()
@
...
<<Figure_6>>=
totals <- data.frame(
profit_under_penalty = dt[,sum(profit_fishing), by='replicate']$V1,
cost_under_penalty = dt[,sum(policy_cost), by='replicate']$V1,
profit_without_penalty = dt[,sum(profit_fishing_alt), by='replicate']$V1,
costs_ignoring_penalty = dt[,sum(policy_cost_alt), by='replicate']$V1)
totals <- melt(totals)
ggplot(totals) + geom_density(aes(value, fill=variable), alpha=.7)
@
...
<<reshapedata, echo=FALSE>>=
df <- read.csv("output.csv")
df2 <- df[c("penalty_fn", "ignore_cost", "assume_cost", "sigma_g", "reduction")] # subset
df3 <- reshape2::melt(df2, id = c("penalty_fn", "sigma_g", "reduction")) 
df3$reduction <- as.factor(df3$reduction)
reduction_table <- read.csv('reduction_table.csv', row.names=1)
reduction_coefs <- melt(data.frame(penalty_fn = rownames(reduction_table), reduction_table), id = "penalty_fn")[c(1,3)]
names(reduction_coefs) <- c("penalty_fn", "coefficient")
@
