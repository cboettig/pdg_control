...
ggplot(df3) + 
  geom_line(aes(sigma_g, value, col=reduction, lty=variable)) +
  #facet_grid(reduction~penalty_fn, scales="free")
  facet_wrap(~penalty_fn, scales="free_y", ncol=1) +
  scale_y_continuous(limits=c(-3e4,3e4))
...
df4 <- df[c(1,2,4,6,7)]
df4 <- plyr::arrange(df4, penalty_fn)
