...
require(reshape2)
XMAX <- 140
policy <- melt(data.frame(stock = x_grid, deterministic=det$D[,1], growth = opt_g$D[,1], meas = opt_m$D[,1], imp = opt_i$D[,1]), id = "stock")
...
ggplot(linear) + 
	...
	geom_jitter(aes(x,y, color=variable), shpae='+') +
