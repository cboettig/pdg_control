...
```{r }
sdp <- SDP_multiple_uncertainty(f, pars, x_grid, h_grid, sigma_g, pdfn, sigma_m, sigma_i)
```

```{r }
mult <- find_dp_optim(sdp, x_grid, h_grid, OptTime, xT, profit, delta, reward=0)
```
...
policy <- melt(data.frame(stock = x_grid, deterministic=det$D[,1], new = mult$D[,1]), id = "stock")
ggplot(subset(policy, stock < 120)) + geom_jitter(aes(stock, stock-x_grid[value], color=variable), shape="+")

dat <- subset(policy, stock < 120)
dt <- data.table(dat)
linear <- dt[,approx(stock, stock-x_grid[value], xout=seq(1,120, length=15)), by=variable]
ggplot(linear) + 
