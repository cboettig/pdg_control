...
pars <- c(1, 2, 6)
p <- pars  # shorthand
K <- p[1] * p[3]/2 + sqrt((p[1] * p[3])^2 - 4 * 
    p[3])/2
xT <- p[1] * p[3]/2 - sqrt((p[1] * p[3])^2 - 4 * 
    p[3])/2  # allee threshold
...
#   generic to types of noise
require(snowfall)
sfInit(parallel = TRUE, cpu = 4)
SDP_Mat <- SDP_by_simulation(f, pars, x_grid, 
    h_grid, z_g, z_m, z_i, reps = 999)
```
...
```r
opt <- find_dp_optim(SDP_Mat, x_grid, h_grid, 
    OptTime, xT, profit, delta, reward = 1)
```
...
sims <- lapply(1:100, function(i) {
    # simulate the optimal routine on a stoch realization
    #   of growth dynamics
...
```r
p1 <- ggplot(dt) + geom_line(aes(time, fishstock, 
    group = reps), alpha = 0.1) + geom_line(aes(time, harvest, 
    group = reps), alpha = 0.05, col = "darkgreen")
stats <- dt[, mean_sdl(fishstock), by = time]
p1 <- p1 + geom_line(dat = stats, aes(x = time, 
    y = y), col = "lightgrey") + geom_ribbon(aes(x = time, 
    ymin = ymin, ymax = ymax), fill = "blue", alpha = 0.1, 
    dat = stats)
p1 <- p1 + geom_abline(intercept = opt$S, slope = 0)
p1 <- p1 + geom_abline(intercept = xT, slope = 0, 
    lty = 2)
print(p1)
```
...
p0 <- ggplot(subset(dt, reps == sample(1:100))) + 
    geom_line(aes(time, fishstock)) + geom_line(aes(time, 
    harvest), col = "darkgreen") + geom_line(aes(time, unharvested), 
    col = "lightblue", alpha = 0.7) + geom_line(aes(time, 
    escapement), col = "darkred", alpha = 0.4) + geom_abline(intercept = opt$S, 
    slope = 0, lwd = 1, lty = 2)
p0
```
...
```r
crashed <- dt[time == OptTime, fishstock == 0, 
    by = reps]
rewarded <- dt[time == OptTime, fishstock > xT, 
    by = reps]
```
...
dt <- dt[crashed]
dt <- dt[rewarded]
setnames(dt, c("V1", "V1.1", "V1.2"), c("total.profit", 
    "crashed", "rewarded"))
```
...
quantile_me <- function(x, ...) {
    ...
    for (i in 1:length(q)) class[x > q[i]] <- i
    class
}
...
```r
p1 <- ggplot(subset(dt, quantile %in% c(1, 5))) + 
    geom_line(aes(time, fishstock, group = reps, color = quantile), 
        alpha = 0.6)
```
