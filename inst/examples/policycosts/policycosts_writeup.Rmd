...
L2_policy <- optim_policy(SDP_Mat, x_grid, h_grid, OptTime, xT, 
                    profit, delta, reward, penalty = L2(0.7586))
````
...
``` {r L1}
L1_policy <- optim_policy(SDP_Mat, x_grid, h_grid, OptTime, xT, profit, delta, reward, penalty = L1(1.52))
````


```{r free_increase}
free_increase_policy <- optim_policy(SDP_Mat, x_grid, h_grid, OptTime, xT, profit, delta, reward, penalty =  free_increase(3.04))
````


```{r free_decrease}
free_decrease_policy <- optim_policy(SDP_Mat, x_grid, h_grid, OptTime, xT, profit, delta, reward, penalty = free_decrease(3.04))
````
...
p0 <- ggplot(dt) +
  ...
  geom_line(aes(time, fishstock, col=penalty_fn, lty=penalty_fn))+ 
