...
Actually, we'll want to draw these from the table in figure4 data at different levels.  
Ideally we can draw the policy out rather than running that again.  
...
Calculate statistics of the kind used in figure 4.  

```{r}
dt[,var(harvest), by=penalty_fn]
dt[,acf(harvest, lag.max=1, plot=F)$acf[2], by=penalty_fn]
```
