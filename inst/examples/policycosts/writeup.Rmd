...
```{r graphing-options, include=TRUE}
opts_knit$set(upload.fun = socialR::flickr.url)
...
Find the value of `c2` that brings each penalty closest to `r reduction*100`% of the cost-free adjustment value:
...
How close has each of these actually gotten to `r 100*reduction`% reduction to NPV0 (which is `r (1-reduction) * npv0`)?
...
sims_free <- list(
  L1 = lapply(reps, function(x) simulate_optim(f, pars, x_grid, h_grid, x0, 
                      L1_policy, z_g, z_m, z_i, 
                      opt$D, profit=profit, penalty=none, seed=seeds[x])), 
  L2 = lapply(reps, function(x) simulate_optim(f, pars, x_grid, h_grid, x0, 
                      L2_policy, z_g, z_m, z_i, 
                      opt$D, profit=profit, penalty=none, seed=seeds[x])),
  fixed = lapply(reps, function(x) simulate_optim(f, pars, x_grid, h_grid, x0, 
...
```{r error_costs_table, results='asis'} 
