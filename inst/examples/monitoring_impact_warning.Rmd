`ro tidy=FALSE, warning=FALSE, comment=NA, message=FALSE, verbose=TRUE, cache=TRUE, cache.path="criticaltransition/" or`
...
``` {r earlywarning2}
acor_tau <- dt[unharvested > 1.5, 
               warningtrend(data.frame(time=time, value=unharvested),
                            window_autocorr),
               by=reps]

var_tau <- dt[unharvested > 1.5, 
              warningtrend(data.frame(time=time, value=unharvested),
                          window_var),
              by=reps]
````
