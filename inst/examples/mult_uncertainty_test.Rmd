...
Tmax = 25
sigmas =c (sigma_g=.3, sigma_m=0.3, sigma_i=0.3)
    ...
    F <- lapply(1:n_h, function(q){  
      t(sapply(1:n_x, function(y){
        ...
        mu <- sum(sapply(1:n_x, function(x)
          f(x_grid[x],h_grid,p) %*% I[q,] *  M[y,x])) # Implementation & Measurement error
        if(mu==0)
          out[1] <- 1 ## Wow, really? should we be applying noise to the f*I*M product or the f?  
        else 
...
```{r mult_uncertainty}
new <- SDP_multiple_uncertainty(f, pars, x_grid, h_grid, OptTime, sigmas = c(sigma_g=sigma_g, sigma_m=0, sigma_i=0), pdfn=pdfn)  
m <- SDP_multiple_uncertainty(f, pars, x_grid, h_grid, OptTime, sigmas = c(sigma_g=sigma_g, sigma_m=0.1, sigma_i=0), pdfn=pdfn)
i <- SDP_multiple_uncertainty(f, pars, x_grid, h_grid, OptTime, sigmas = c(sigma_g=sigma_g, sigma_m=0, sigma_i=0.1), pdfn=pdfn)
```
...
```{r policyfn_data}
require(reshape2)
policies <- melt(data.frame(stock=x_grid, old = x_grid[opt$D[,1]], new = x_grid[new$D[,1]], meas = x_grid[m$D[,1]], imp = x_grid[i$D[,1]]), id="stock")
```
