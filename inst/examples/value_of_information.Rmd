...
require(reshape2)
policy <- melt(data.frame(stock = x_grid, det = det$D[,1], g = g$D[,1], m = m$D[,1], i=i$D[,1], gm = gm$D[,1], gi = gi$D[,1], gmi = gmi$D[,1]), id = "stock")
...
value <- melt(data.frame(stock = x_grid, det=det$V, g = g$V, m = m$V, i=i$V, gm = gm$V, gi = gi$V, gmi = gmi$V), id = "stock")
