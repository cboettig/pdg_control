...
``` {r tidy, cache=FALSE}
sims <- list(base = base$sims, full = full$sims, reckless=reckless$sims, cautious=cautious$sims)
...
``` {r  fig.cap="the induced dynamics in the stock size over time, for all replicates, by scenario", cache=plotcache}
p1 <- ggplot(dt) + geom_abline(intercept=opt$S, slope = 0) 
p1 + geom_line(aes(time, fishstock, group = reps), alpha = 0.1) + facet_wrap(~uncertainty)
````

``` {r fig.cap="The profits made in each time interval of a single replicate, by scenario", cache=plotcache}
ggplot(subset(dt,reps==1)) +
...
``` {r fig.cap="the distribution of profits by scenario", cache=plotcache}
