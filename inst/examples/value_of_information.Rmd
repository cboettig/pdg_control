...
``` {r stock, fig.cap="the induced dynamics in the stock size over time, for all replicates, by scenario"}
p1 <- ggplot(dt) 
p1 + geom_line(aes(time, fishstock, group = reps), alpha = 0.1) + facet_wrap(~uncertainty)
````

``` {r profits_time, fig.cap="The profits made in each time interval of a single replicate, by scenario"}
ggplot(subset(dt,reps==1)) +
...
``` {r profits, fig.cap="the distribution of profits by scenario"}
