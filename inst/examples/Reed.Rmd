...
With parameters `K` = `r K` and `r` = `r r`.
...
``` {r policyfn_plot}
require(reshape2)
policies <- melt(data.frame(stock=x_grid, S = x_grid[opt$D[,1]], D = x_grid[det$D[,1]], Stationary = x_grid[s_opt$D]), id="stock")
q1 <- ggplot(policies, aes(stock, stock - value, color=variable)) + geom_point(alpha=.5) + xlab("stock size") + ylab("escapement") 
q1
````
