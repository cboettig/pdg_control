...
policy <- melt(data.frame(stock = x_grid, det = det$D[, 
    1], low = low$D[, 1], g = g$D[, 1], m = m$D[, 1], i = i$D[, 
    1], gm = gm$D[, 1], gi = gi$D[, 1], mi = mi$D[, 1], gmi = gmi$D[, 
    1]), id = "stock")
policy <- melt(data.frame(stock = x_grid, low = low$D[, 
    1], g = g$D[, 1], m = m$D[, 1], i = i$D[, 1]), id = "stock")


ggplot(policy) + # geom_point(aes(stock, stock-x_grid[value],
#   color=variable)) +
geom_smooth(aes(stock, stock - x_grid[value], 
    color = variable)) + ylab("escapement")
```

![plot of chunk sethiplots](http://farm9.staticflickr.com/8014/7443817062_b1ebc64431_o.png) 
