...
Figures S9-S12 illustrate that the 
...
```{r Figure_S9, dev=c("pdf", "svg", "png"), fig.width=7, cache=FALSE, fig.cap="Identical analysis to Figure 3 of the main text, but using a calibration corresponding to a 10% reduction in NPV.  This results in a less pronounced version of the same qualitative pattern."}
...
```{r Figure_S10, dev=c("pdf", "svg", "png"), fig.width=7, cache=FALSE, fig.cap="Identical analysis to Figure 3 of the main text, but using a calibration corresponding to a 30% reduction in NPV.  This results in a more pronounced version of the same qualitative pattern."}
...
```{r Figure_S11, dev=c("pdf", "svg", "png"), fig.width=5, fig.height=6, cache=FALSE, fig.cap="Identical analysis as in Figure 4 of the main text, but using a calibration to 10% reduction in NPV rather than 25%.  The qualitatively similar pattern helps illustrate that these results are not specific to the value of the reduction chosen in the main text."}
p4 <- ggplot(low$f4df, aes(x = penalty_fraction, fill=penalty_fn, col=penalty_fn)) + 
  coord_cartesian(xlim = c(0, .3)) + 
  scale_color_discrete(labels=relabel, name="cost model") + 
  scale_fill_discrete(labels=relabel, name="cost model") 
...
p4a <- p4 + 
  ...
  stat_summary(aes(y=autocorrelation), fun.y = mean, geom = "line") + 
  xlab("") + 
  labs(title="Summary statistics under a 10% reduction in NPV")
...
```{r Figure_S12, dev=c("pdf", "svg", "png"), fig.width=5, fig.height=6, cache=FALSE, fig.cap="Identical analysis as in Figure 4 of the main text, but using a calibration to 30% reduction in NPV rather than 25%.  The qualitatively similar pattern helps illustrate that these results are not specific to the value of the reduction chosen in the main text."}
p4 <- ggplot(high$f4df, aes(x = penalty_fraction, fill=penalty_fn, col=penalty_fn)) + 
  coord_cartesian(xlim = c(0, .3)) + 
  scale_color_discrete(labels=relabel, name="cost model") + 
  scale_fill_discrete(labels=relabel, name="cost model") 
...
p4a <- p4 + 
  ...
  stat_summary(aes(y=autocorrelation), fun.y = mean, geom = "line") + 
  xlab("") + 
  labs(title="Summary statistics under a 30% reduction in NPV")
