...
```{r Figure_1, fig.cap="Quota on Blue Fin Tuna Catch "}
...
<!--NOTE ci notation is confusing because it does not usually refer to c0 or c4 -->
...
present value of fishery, broadly following the example of  @Bovenberg_2008.
...
fig3_df <- as.data.frame(subset(dt, replicate=='rep_1'))
...
ggplot(fig3_df, aes(time, value, col=baseline)) +
  geom_line() +
  facet_grid(variable~penalty_fn) + 
  labs(x="time", y="stock size", title = "Example Stock & Harvest Dynamics")  

write.csv(fig3_df, "components/data/figure3.csv")
```


Figure 3 illustrates how the different forms of adjustment cost can
impact the optimal policy (lower row) and corresponding stock sizes,
which are measured here before harvest ($h_t$ being determined after
observing stock size $x_t$). Each panel is generated against the same
sequence of random shocks so that they can be compared directly.
...
(linear costs, $\Pi_1$), the bottom-left panel of Figure 3 shows a typical pattern, in which
the optimal policy tends to avoid small policy adjustments, resulting in
periods of constant policy followed by sudden bursts of adjustment.
This results in a relatively step-like policy pattern. In contrast, the
second formulation (quadratic costs, $\Pi_2$) disproportionately penalizes large policy
...
The comparisons shown in Figure 3 are for one realization (i.e. a
...
autocorrelation of harvests and stock sizes through time. <!-- NOTE Are we showing cross-correlations or not? --> <!-- See figure 4, probably not-->
...
scale linearly with the size of proposed policy changes: in the 100 replicates
of data presented above the harvest policy is strictly positive and identical in consecutive intervals
...
these costs ($c_1$) increases. 



<!-- to include cross.correlation, drop subset call and facet_wrap by variable. But shows no pattern-->
...
```{r Figure_5}
ggplot(hist_dat) + geom_density(aes(value, fill=variable), alpha=0.6)+facet_wrap(~penalty_fn)
...
taken across 100 replicate runs are shown are shown in Figure 5
...
```{r Figure_6, fig.cap="Relative cost of ignoring adjustment costs when they are present ('ignoring', red) vs assuming the adjustment costs when they are absent ('assuming', blue), shown by penalty function over increasing magnitudes of cost.  All values are relative to the cost-free adjustment optimum, NPV0.  In each case, it is less costly to assume adjustment costs are present when they are not than it is to ignore them when they are present. The severity of the latter mistake increases sharply as adjustment costs make a larger fraction of the total costs."}
...
Figure 6 compares these different contributions, namely the $NPV$ lost by
...
adjustment costs apply or are assumed to apply $NPV_0({\bf h_0^*})$. 
...
When policy costs are present, assuming the wrong functional form may or may not be worse than ignoring the presence of policy costs all together.  Figure 7 considers several such cases.  The X axis labels indicate first which policy is being used to drive the harvest decisions, and then which penalty is actually being applied.  Thus `L1_L2` indicates a policy calculated under the linear $L_1$ costs, when the reality involves quadratic $L_2$ costs.  Though the coefficients of the $L_2$ costs have been scaled such that the net effect is the same, the inferred policies differ.  As a result, the value is slightly more than half of the optimal free solution when the expected reduction should only be 10% less than optimal free. In this case, ignoring the costs all together is still worse than using L1 costs -- the red bar is lower than the blue.  

For fixed costs, the reverse is true.  Because fixed costs increase the volatility of the optimal solution while L2 costs decrease it, simply ignoring the cost of adjustment all together results in a higher value than either assuming costs are quadratic $L_2$ when they are fixed, or assuming they are fixed when they are quadratic.  

Nevertheless, assuming quadaratic costs is always the most conservative option -- scenarios in which true costs are quadratic (regardless of what cost structure is assumed by the policy) do significantly worse than those that are not (i.e. all cases that assume quadratic but actually have linear or fixed costs).  

These patterns hold across the different size reductions shown (from 10% to 25%), though net present value becomes negative under the quadratic form for the larger penalties as costs paid for adjustments exceed profits derived from harvest.  (Though optimal solutions will always avoid negative net present values, these scenarios in which policies are applied under conditions other than assumed in the optimality calculation have no such guarentee.) For each scenario, results are averaged over 100 replicates.  



```{r Figure_7, fig.cap="Mismatches. X axis labels indicate the model used to derive the policy, followed by the model used to simulate the penalty. Blue bars show the net present value of that scenario.  Compare to red bars, which show the consequence of using a policy that ignores costs all together in the same scenario (i.e. costs as indicated by second code), rather than assuming the policy indicated by the first code in the pair.  All values are scaled to a fraction of the adjustment free net present value.  Successive panels show increasing reduction of the net present value by increasing penalty coefficients: e.g. a policy in the first panel should lose only 10% of its net present value to adjustment costs if it had the correct form of those costs."}
...
Laukkanen, M. 2003. Cooperative and non-cooperative harvesting in a stochastic sequential fishery. J Environmental Economics and Management 45, 454-473.

Lewis, T.R. 1981. Exploitation of a renewable resource under uncertainty. Canadian Journal of Economics 14, 422-439.

McFadden, D. 1975. The revealed preference of a government bureaucracy: theory. Bell Journal of economics 6, 401-416.

McFadden D. 1976. The revealed preference of a government bureaucracy: empirical evidence. Bell Journal of Economics 7, 55-72.

McGough, B., Plantinga, A.J., Costello, C. 2009. Optimally managing a stochastic renewable resource under general economic conditions. B.E. Journal of Economic Analysis & Policy 9, Iss 1, Article 56.


Ralston S., Punt, A.E., Hamel, O.S., DeVore, J.D., Conser, R.J. 2011. A meta-analytic approach to quantifying scientific uncertainty in stock assessments. Fish. Bull. 109: 217-231.
[NOTE WE CAN CITE STEVE’S PAPER FOR THIS POINT + I LIKE THE PAPER. BUT THE POINT IS PRETTY BEING CLAIMED IS PRETTY ELEMENTARY AND IT’S POSSIBLE WE COULD CITE A STOCK ASSESSMENT TEXTBOOK INSTEAD.]
...
ALSO REFERENCE TO ADD TO METHODS SECTION

When introducing fixed fee – conceptual analogues to the set-up cost of Reef (1974) and Spulber (1982) but here the fee is associated with a change in harvesting ($h_t\neq h_{t-1}$) and not with harvesting {\em per se} ($h_t>0$) as in those studies.
...
Spulber, D.F. 1982. Adaptive harvesting of a renewable resource and stable equilibrium. In: Essays in the Economics of Renewable Resources. L.J. Mirman and D.F. Spulber (eds.) North-Holland Publishing, Amsterdam. pp. 117-139.
