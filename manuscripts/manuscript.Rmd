...
the adjustment cost free optimum, depending on the structure of the cost function.  In each
case, the optimal solution without any adjustment cost is shown with
...
While the optimization ensures that on average, the policy in the first
simulation should outperform that of the second, it does not tell us by
how much.  These simulations thus allow us to quantify this effect.

We provide 100 replicates of each kind of scenario for each of the
three forms $\Pi_1$, $\Pi_2$ & $\Pi_3$.  For each, we record (a)
the dockside profits derived from fishing as well as (b) any fees paid
to make adjustments to the policy.  In the first kind of simulation,
dockside revenues are almost less than in the second for a given
stochastic replicate, since the latter optimizes the expected NPV.
However, the costs paid for adjustment are almost always less in the
first scenario as well, since this solution explicitly tries to avoid
higher adjustment costs.  We visualize this pattern in the distributions
shown in Figure 5 (focusing on $\Pi_1$ for simplicity, as the pattern
in $\Pi_2$ and $\Pi_3$ is similar, see Supplementary Material.)
...
through higher profits it must do so through lower costs.  Interestingly,
for this set of parameters very little is lost in dockside revenue.
This suggests that the impact of assuming costs are present when they
are not is rather small: since the dockside revenue is none-the-less
...
We visualize the differences between these two errors directly in Figure
6. Subtracting revenue from adjustment costs and considering only the mean
across the replicates, we can dervive the expected $NPV$ in each case.
...
variation in the catch quotas that would be recommended. Also, the catches
and remaining stock sizes corresponding to optimal management become more
autocorrelated in time, because it takes the fishery longer to harvest
