...
apples })
write.csv(reduction_table, "components/data/reduction_table.csv")
````
...
```{r table-calc}
source("componetns/compute_error_table.R")
...
df4 <- error_df[c(1,2,4,6,7)]
plyr::arrange(df4, penalty_fn)
pandoc.table(df4)
```
