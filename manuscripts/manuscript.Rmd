...
The corresponding figures can be found in the Supplemental Materials.
...
of the differences is greatest when the penalty is larger. Note that the supplementary material provide
...
```{r Figure_6, fig.cap="Relative cost of ignoring adjustment costs when they are present ('ignoring', red) vs assuming the adjustment costs when they are absent ('assuming', blue).  All values are relative to the cost free adjustment optimum, $NPV_0$.  In each case, it is less costly to assume adjustment costs are present when they are not than it is to ignore them when they are present.Supplementary Figure A6 shows how the magnitude of this effect changes with the size of the reduction to $NPV$.", dev=c("pdf", "svg", "png"), fig.width=7, cache=FALSE}
