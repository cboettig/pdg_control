...
a recent stock assessment [@ICCAT20XX].  The bottom-left panel 
...
the observed quota stability. Such patterns of variable stocks managed
...
shown in the right-hand panels [@SEDAR2013]. More generally, reviews by @Biais1995 and
...
here are not sensitive to the specific choices for these parameter values.
...
in the _previous_ time step: a total of $S \times Q \times Q$ configurations.
...
In the Supplementary Material, we compare our results with 
...
<!-- State conclusion of this supplement here! -->

Comparing apples to apples
--------------------------

Before we can compare the policy dynamics resulting from each of these
functional froms, we must be careful to ensure that we are only varying
the _structure_ of the adjustment penalty and not the overall magnitude.
At the extremes, a large enough adjustment cost can ensure a constant 
policy, regardless of the structure; while a small enough one can be ignored
regardless.  Defining this calibration correctly is thus key to the entire
analysis.  Fortunately, the optimal control framework provides a completely
natural way to make this comparison: in terms of the _economic value_
(measured by $NPV$) of the stock in each case.  We will calibrate this relative
to economic value of the stock when adjustments to the policy are free ($NPV_0$).
...
choices of $c_i$ so that each has a comparable impact on the optimal NPV
fishery.  
...
dashed vertical lines in Figure 2 map the needed cost coefficient $c_i$,
for each penalty function, such that the fishery is worth `r 100*(1-reduction)`% of its
unconstrained value when optimally managed. We focus our examination of the models on how
the type and severity of policy adjustment costs affects the optimal management strategy.  
...
The first panel shows a typical pattern resulting from linear adjustment costs ($\Pi_1$).
The optimal policy tends to avoid very small policy adjustments, resulting in
periods of a constant policy followed by sudden bursts of adjustment.
...
adjustments. The corresponding optimal policy is typified by the middle panel, responding
to each of the fluctuations in stock, made by the cost-free policy, but with smaller 
magnitude response than the equivalent cost-free optimal solution. This results
...
realization (i.e. a particular sequence of random number draws representing
environmental variability) and are made for one particular magnitude of 
policy adjustment costs (Calibrated to equal to `r reduction *100`% of the 
maximum expected net present valuein the base case ($NPV_0({\bf h_0^*})$,
as discussed in Figure 2, permitting comparisons between different forms).
To demonstrate this, we solved for the optimal policy under each of the
three adjustment cost scenarios for `r length(c0)` different penalty 
coefficients. For each resulting policy, we then simulated 50 stochastic
replicates of the stock dynamics managed under that policy.  To summarize
the patterns shown in Figure 3 and characterize the impact of increasingly
large fraction of the economic value (NPV) being consumed by policy
adjustment costs, we examine the response of several summary statistics
to increasing adjustment penalties (as a fraction of the adjustment-free value)
in Figure 4.  We show the impact of policy adjustment costs on the
variance & autocorrelation of the harvests through time. 


```{r Figure_4, fig.cap="Variance and autocorrelation of harvest dynamics, as a function of the penalty coefficient.  Adjustment penalty is calculated as percentage of the maximum expected $NPV$ in the basic case with no policy adjustment costs.", fig.height=8, fig.width=8, cache=fig.cache, dev=c("pdf", "svg", "png")}
...
not. To do so, we run two kinds of simulations:

1. Simulations in which the optimal policy accounts for adjustment costs, 
   and those costs are present (in the form assumed),
2. Simulations in which the optimal policy _does not_ account for adjustment costs,

While the optimization guarentees that on average, the policy in the first simulation
should outperform that of the second, it does not tell us by how much.  These simulations
thus give us a lense to examine the magnitude of this effect.

We provide 100 replicates of each kind for each of the three forms
$\Pi_{1-3}$ (in which each ensemble of 100 replicates uses the same sequence of random seeds).
For each, we record (a) the dockside profits derived from fishing as well
as (b) any fees paid to make adjustments to the policy.  In the first kind of 
simulation, dockyard profits are almost less than in the second for a given stochastic
replicate, since the latter optimizes the expected dockside NPV.  However, the costs
paid for adjustment are almost always less in the first scenario as well, since
this solution explicitly tries to avoid higher adjustment costs.  We visualize this
pattern in the distributions shown in Figure 5 (focusing on $\Pi_1$ for simplicity,
as the pattern in $\Pi_2$ and $\Pi_3$ is similar, see Supplementary Material.)


Figure 5 shows that the difference in dockside revenue (labelled "profits" in the figure) 
between these two scenarios 
(green vs purple distributions) is very small, while a larger difference exists between
the costs paid for making policy adjustments (red vs blue). This is predicted by the optimization: 
the first scenario must outperform (or at least equal) the NPV of the second scenario
in aggregate, and as it cannot do so through higher profits it must do so through lower
costs.  Interestingly, for this set of parameters very little is lost in dockside revenue.
This suggests that the impact of assuming costs are present when they are not is rather small:
since the dockside revenue is none-the-less near optimal.  This also suggests that the 
converse error: ignoring costs when they are present, is more severe.  
...
We visualize the differences between these two errors directly in Figure 6. Subtracting revenue from
adjustment costs and considering only the mean across the replicates, we can dervive the expected $NPV$ 
in each case.  To standardize against a common baseline, we will express this value as a fraction of 
the cost-free optimum, $NPV_0$. Note that we have calibrated the optimal solutions to anticipate that
`r reduction*100`% of their $NPV$ would go into adjustment costs. When these adjustment costs are not 
actually present (blue bars, Fig 6), we thus expect a $NPV$ higher than the anticipated `r (1 -reduction)*100`%
but not quite as high as the optimal policy for that scenario (100%).  When adjustment costs are present
but the policy assumes that they are not (red bars), we would expect the $NPV$ to be less than `r (1 - reduction) * 100`%
of $NPV_0$ (since that's as good as the optimal solution for this scenario can do).  

Figure 6 shows that across each penalty form $\Pi{1-3}$, the economic impact
of ignoring adjustment costs when they are present is higher than accounting
for them when they are absent. This holds even after accounting for effect 
described above: the red bars are consistently well below (`r (1-reduction)`),
while the blue bars are barely below 1.
...
In the Supplementary Materials we show that pattern holds across different
severity of adjustment costs (robust to choice of $c_i$), though the larger
the adjustment cost, the greater the difference between these two errors.

Note that this analysis does not consider the possibility that an adjustment cost
is present, but of a different functional form than the policy assumes.  Not surprisingly,
the consequences of managing under the wrong functional form can be as or more severe than
simply ignoring adjustment costs in the optimization all-together.  We illustrate several
of these mismatch errors in the Supplementary Material. 
...
<!-- Comment on increasing variance or increasing autocorrelation from changes in policy costs / optimal management-->
