...
table1 <- arrange(subset(error_df, sigma_g == 0.01)[who], penalty_fn) 
pandoc.table(table1, caption=caption)

table1_long <- melt(table1, id = c('penalty_fn', 'reduction'))
table1_long$reduction <- as.factor(table1_long$reduction)
ggplot(table1_long, aes(penalty_fn, value, fill = variable)) + geom_bar(stat="identity", position="dodge") + facet_wrap(~ reduction) + theme_bw()
```
