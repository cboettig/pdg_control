...
keywords: [optimal control, fisheries, ecological management, adjustment costs]

layout: review,12pt,author-year,titlepage
linenumbers: true
bibliography: components/references.bib
csl: components/ecology.csl
documentclass: components/elsarticle
geometry: [verbose,letterpaper,tmargin=2.54cm,bmargin=2.54cm,l
margin=2.54cm,rmargin=2.54cm]
endfloat: true
doublespacing: true
...
output:
  pdf_document:
    ...
    keep_tex: true
    includes:
      after_body: appendix_description.tex 
...
declining by `r 100 * (1 - min(tuna$stock) / max(tuna$stock))`% between
1982 and 1991, the quota was not changed during
...
under far less variable quotas are extremely common, as in the instance of 
the management of south Atlantic snowy grouper (_Epinephelus niveatus_) 
...
as responsive as this constant escapement policy assumes. In this paper, we consider a
...
what functional form they should take and that it will require substantial 
empirical work to estimate that. Therefore, we investigate three candidate
...
growth parameters are chosen so that the equilibrium biomass for the equivalent
deterministic model without harvesting is `r (pars[1]-1)/pars[2]`; (specifically, $A=$ `r pars[1]` and
...
In Appendix A we illustrate that the qualitative patterns observed
...
provided as a supplementary R package to the paper, ([github.com/cboettig/pdg_control](https://github.com/cboettig/pdg_control), @zenodo_copy), along with scripts for replicating 
the analyses presented here. To address concerns about computational reproducibility [@Boettiger2015],
a Docker container image of the software environment is also provided.
...
In Appendix A, we compare our results with more
...
ggplot(fees, aes(c2, (npv0-value)/npv0, lty=variable, color=variable)) + 
  ...
  scale_linetype_discrete(labels=relabel, breaks = c('L1', 'L2', 'fixed'), name = 'cost model') + 
  scale_color_discrete(labels=relabel, breaks = c('L1', 'L2', 'fixed'), name = 'cost model') + 
  annotate("text", mean(c2), reduction+.02, label = "25% reduction in NPV")
...
$c_i$, for each penalty function, such that the fishery is worth `r 100*(1-reduction)`%
of its unconstrained value when optimally managed. To demonstrate that the results
shown here are independent of this choice of calibrating to a `r 100*(1-reduction)`%
reduction, we conduct the same analysis with a 10% and 30% reduction rate as well.
The corresponding figures can be found in Appendix A.
...
ggplot(dt3, aes(time, value, col=variable, lty=variable)) +
  ...
  labs(x="time", y="harvest", title = "Example Harvest Dynamics")  +
...
sizes can be seen in Appendix A.  Each panel is generated against the
...
resulting policy, we then simulated `r length(reps)` stochastic replicates of the stock
...
```{r Figure_4, fig.cap="Variance and autocorrelation of harvest dynamics, as a function of the penalty coefficient.  Adjustment penalty is calculated as percentage of the maximum expected NPV in the basic case with no policy adjustment costs.", fig.height=6, fig.width=5, cache=fig.cache, dev=c("pdf", "svg", "png")}
...
p4 <- ggplot(f4df, aes(x = penalty_fraction, fill=penalty_fn, col=penalty_fn)) + 
...
Figure 4 shows these qualitative patterns still hold at when the models are calibrated 
to a range of different reductions in NPV, from 0% to 30%, though naturally the magnitude
of the differences is greatest when the penalty is larger. Note that results shown in Appendix A provide
further evidence that these patterns are independent of the exact reduction shown by providing
results analogous to Figure 3 (A8, A9) and Figure 5 (A10, A11), and Figure 6 (A6).
...
We will perform `r length(reps)` replicate simulations of this for each of the three
...
```{r Figure_6, fig.cap="Relative cost of ignoring adjustment costs when they are present ('ignoring', red) vs assuming the adjustment costs when they are absent ('assuming', blue).  All values are relative to the cost free adjustment optimum, $NPV_0$.  In each case, it is less costly to assume adjustment costs are present when they are not than it is to ignore them when they are present. Appendix A, Figure A6 shows how the magnitude of this effect changes with the size of the reduction to $NPV$.", dev=c("pdf", "svg", "png"), fig.width=7, cache=FALSE}
...
change being implemented ($\Pi_2$).  In Appendix A we
...
per unit effort is increasing in the amount of effort expended [@Brown1974;
@Lewis1981; @McGough2009]; (see Appendix A for a summary of the
...
model assumptions [@Brown1974; @Lewis1981; @Feichtinger1994; @Wirl1999; @McGough2009],
...
annual quota that does not change through time.
...
same as using the wrong functional form when it is present.  In Appendix A
...
e.g. by assuming regulated open access conditions [@Homans1997],
or models that derive policy recommendations endogenously by modeling strategic 
...
collapse. This includes models with larger stochasticity as well as model
formulations that can permit alternative stable states.
Examining risks of stock collapse is not possible with the
Reed model formulation that we followed here (both cases are inconsistent
with the assumptions of @Reed1974) but would be worthwhile
...
Iadine Chades and Mandy Karnauskas, as well as the two anonymous reviewers
whose feedback strengthened this paper.  We also acknowledge the support of
NSF Grant DBI-1306697 to CB and ARC DECRA (DE130100572) & the
ARC Centre of Excellence for Environmental Decisions to MB.
