...
fraction_no_shift <- function(x){
 nonzeros <- which(x > 0)
 x <- x[nonzeros]
 sum(x[2:length(x)] - x[1:length(x)-1] == 0)/length(x)
}
...
As with any modeling analysis, our formulation makes many assumptions. For example, we assume that the fishery in question is being optimally managed by a policy-maker who acts as the 'sole owner' of the stock. This approach is different to models that assume fisheries are not well-managed, e.g. by assuming open access conditions (references), or ones that derive policy recommendations endogenously by modeling interactions between different stakeholders (game theory references). As such, we anticipate that our approach will be more relevant to some fisheries, particularly domestic fisheries in developed countries that are subject to relatively strong regulatory regimes, than to others (e.g., artisanal fisheries that are subject to weaker regulation). A second important assumption we made is that the fishery manager observes the current stock size correctly when setting the annual quota (see Clark and Kirkwood, Williams + more for alternative specifications). However, stock assessments struggle to predict current cohort sizes based as they are on observing past catch history (Ralston et al). We might anticipate that when uncertainty regarding reported recruitment pulses is included, we see the tendency for an optimizing manager to moderate their policy responses to initial reports of particularly good recruitment years increase yet further.

One obvious research avenue suggested by our models is that of empirically estimating costs of policy adjustment. A direct estimation approach could quantify some sources of policy adjustment costs, e.g. costs to processing plants that arise from having more variable catches. However, other more intangible sources of policy adjustment costs, e.g. preferences of policy-makers or different stakeholders for less variable quotas, might be missed. An alternative, more holistic, approach would be to apply revealed preference methods to fisheries management agencies themselves, in the tradition of McFadden (1975, 1976). Such an analysis would involve comparing quotas that were set relative to stock sizes as they were estimated at the time each management decision was taken to try to infer what objective managers were maximizing. Some stocks are assessed and some quotas revisited more frequently than others, which provides relevant patterns of variation in policy environments that econometric analyses of this type could try to exploit.

A worthwhile modeling extension suggested by our results would be to examine the implications of policy adjustment costs for risks of stock collapse. Examining risks of stock collapse is not possible with the Reed model-formulation that we followed here. But using an alternative approach also would be worthwhile in light of our finding that the variance and autocorrelation in stock sizes through time can be affected when accounting for costs of policy adjustment. Both are properties known in other modeling settings to be associated with changes to the risk of extinction or of transitions between alternative stable states. In such an analysis, it would be interesting to explore more complicated patterns of environmental variability than we have here (e.g., colored environmental noise). Another obvious extension would be to combine the different functional forms that we consider rather than studying them in isolation as we have here. Finally, a third would be to consider how policy adjustment costs affect approach paths to the long-run management outcomes, which provide the focus for this study.

Conclusions
===========

Lessons for modelers - ignoring reality of policy costs leads to less effective management (decreased value)

Lesons for policy makers - decreasing adjustment costs not only saves direct costs but permits higher value solutions.
