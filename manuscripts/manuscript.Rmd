...
```{r summary-stat-calc, dependson="sim_at_each_apple"}
...
```{r Figure_2, dependson="tidy", fig.cap="Example realization of optimal harvesting strategy and resulting fish stock sizes under the different functional forms of adjustment costs.", fig.height=3, fig.width=8}
...
Same comments as before on labeling the curves and detailing in the figure captions. Axis labels and things all also need work. - Paul
-->
<!--
...
ggplot(subset(figure4_df, statistic != "cross.correlation"), aes(penalty_fraction, value, fill=penalty_fn, col=penalty_fn))  +
  stat_summary(fun.y = mean, 
               fun.ymin = function(x) mean(x) - sd(x), 
               fun.ymax = function(x) mean(x) + sd(x), 
               geom = "ribbon", alpha=0.3, colour=NA) +
  stat_summary(fun.y = mean, geom = "line") + 
  facet_grid(statistic ~ timeseries, scale="free") + theme_bw()
...
ggplot(table1_long, aes(penalty.fn, value, fill = variable)) + geom_bar(stat="identity", position="dodge") + facet_wrap(~ reduction) + theme_bw()
```
