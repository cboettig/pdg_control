...
ggplot(fees, aes(c2, (npv0-value)/npv0, lty=variable, color=variable)) + 
  ...
  annotate("text", mean(c2), reduction+.01, label = "Calibration for 25% reduction in NPV")
...
ggplot(filter(table1_long, reduction == reduction),
  ...
  scale_x_discrete(labels = c('L1' = substitute(paste(Pi[1])),
                              ...
                              'fixed' = substitute(paste(Pi[3])))) + 
  xlab("Cost model") + 
  ylab(expression(NPV[0])) + theme_tufte(base_size=16)
