...
abstract: |
          ...
          they are present.
layout: preprint      # review, 11pt
---

<!-- CODE -->


```{r libraries, output="hide", include=FALSE, cache=FALSE}
rm(list=ls())
library("pdgControl")
library("reshape2")
library("ggplot2")
...
```{r reed, dependson=c("setup", "profit_model")}
...
``` {r bigloop, dependson=c("setup", "reed", "fees", "profit_model", "c2_grid")}
...
```{r apples, dependson=c("quadcosts", "reduction")}
...
```{r npv_table, dependson="print_npv", results='asis'} 
...
```{r simulate_policy, dependson=c("policynames", "apples")}
reps <- 1:100
...
to respond to variations in ecosystem dynamics [@Walters1978; @Armsworth2012]. 
...
@Patterson2007; @PattersonResimont2007; @Sanchirico2008].
...
@Patterson2007 document many cases where changes in catch
...
Eqn. [eq:annual_netrevenue], $p$ is the price per unit harvest and
...
equation ([eq:state_equation]) define a stochastic dynamic programming
...
<!--PRESUMABLY YOU USED A FINITE TERMINAL TIME IN WHICH CASE
...
Taken together with the state equation (Eqn. [eq:state_equation]), each
...
<!-- NOTE Will need to do the runs and generate the figures for that one  -->
...
adjustments to the controls: $\Pi_4 (N_t,h_t) = p h_t -  c_0 E_t-c_4E_t^2$. Additional costs of this
...
<!-- ![**Expected net present value by functional form of penalty.** Horizontal axis shows the coefficient $c_i$ governing the magnitude of the policy cost, while vertical axis shows fraction of the maximum expected net present value dissipated by the policy cost.  The horizontal line indicates a value of the stock that is 80\% of the maximum expected value in the absence of policy adjustment costs, $NPV_0({\bf h_0^*})$. Selecting the coefficient $c_i$ corresponding to this value in each functional form allows us to make consistent comparisons across the different functional forms of policy costs. ](figure/Figure_1.pdf)

-->

<!-- NOTE
Figure needs tidying up to be submission ready. Resize it – can be smaller, no color needed, bigger TickLabels, no background shading or grid. It also need the relevant horizontal and vertical lines marking to show the calibration. Choose a % reduction that corresponds to that you are choosing for what is currently labeled Figs 2&3 (sample paths) and Fig 8 – histograms. We should then adjust the text to talk to what is shown in the figure consistently – e.g. is is 25% loss or 75% of the value, etc. -->
...
<!-- If, as discussed over email, we are going to go with just one in the main text blown up a bit bigger to make it more visible, I need to have that figure to be able to write this text around it. Note I would still present the full set in the SI. Rather than trying to recover old random seeds if not stored. Just run one anew. -->
<!-- 
For the figure in the main text, I would make a single figure with six plates – the three different adjustment costs are the columns. One row is the effect on optimal harvests and one the effect on optimal stock sizes. -->
...
3 shows the corresponding stock sizes, which are measured here WHEN 

<!-- At what time relative to the dynamics in equation 1, and I would be setting up a comment why they don’t see something similar to the constant escapement policy – this is just a census time issue, but I would say it explicitly given the attention we shine on the REED result upfront.
-->

. In 
each case, the optimal solution without any adjustment cost is shown in
black and the policy induced by optimization under the given adjustment
penalty (equivalent to a 25% reduction in maximum expected NPV) is
overlaid in blue.
...
<!--
![Individual realizations of optimal harvesting strategy under the different functional forms of adjustment costs.](figure/Figure_2)

![Individual realizations of optimal stock size strategy under the different functional forms of adjustment costs](figure/Figure_3)
-->
...
adjustment costs equal to 25% of the maximum expected net present value <!-- Match to that shown in Figure 1. -->
...
cross-correlation of harvests and stock sizes through time. <!-- Are we showing cross-correlations or not?-->
...
<!--
Assuming we aren’t showing cross-correlations, I think we can probably fit these into a single four subplot figure. Cross-correlations would be a little harder but just using 5 subplots from what could have been a 6 subplot figure would also work, because a journal could just put the caption into the empty space.

Same comments as before on labeling the curves and detailing in the figure captions. Axis labels and things all also need work.
-->


<!--
![Figure 4a: Variance of optimal harvest rates through time as a function of size of adjustment penalties, the latter being calculated as percentage of the maximum expected NPV in the basic case with no policy adjustment costs.](figure/Figure_4a.pdf)

![Figure 4b: Autocorrelation of optimal harvest rates through time as a function of size of adjustment penalties, the latter being calculated as percentage of the maximum expected NPV in the basic case with no policy adjustment costs](figure/Figure_4b.pdf)

![Figure 5a: Variance of optimal stock sizes through time as a function of size of adjustment penalties, the latter being calculated as percentage of the maximum expected NPV in the basic case with no policy adjustment costs.](figure/Figure_5a.pdf)

![Figure 5b: Autocorrelation of optimal stock sizes through time as a function of size of adjustment penalties, the latter being calculated as percentage of the maximum expected NPV in the basic case with no policy adjustment costs.](figure/Figure_5b.pdf)
-->
...
<!--
Obviously we have presentation choices to make here. I think there is plenty of scope either to compact this table somehow (rows are penalty functions)  x columns are percent reduction.
