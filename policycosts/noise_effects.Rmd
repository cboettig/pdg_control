...
# adjust by discount rate
tmp$profit <- tmp$profit/(1+delta)^tmp$time
tmp2$profit <- tmp2$profit/(1+delta)^tmp2$time
...
```{r}
identical(p1,p2)
c(low_noise = sum(tmp$profit), high_noise = sum(tmp2$profit), low_theory = value1, high_theory=value2)
```
