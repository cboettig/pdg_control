...
# Calculating if information costs are essentially addititve or not.   
...
The first scenario considers managing with the completely deterministic rules when in fact both measurement and growth are stochastic.  
...
We simulate 100 replicates of this system in which measurement and growth are actually stochastic.  We will used a fixed seed so that we can compare these replicates to simulations under different conditions.    

```{r }
z_g <- function() rlnorm(1,  0, 0.15) 
z_m <- function() rlnorm(1,  0, 0.15) 
z_i <- function() 1 
set.seed(42)
...
As before, we simulate 100 replicates using the same random number sequence, now under this case where the noise in growth is being accounted for by the management, but not the measurement noise.  
...
```{r }
sigma_m <- 0.15    # Noise in population growth
z_g <- function() 1
z_m <- function() rlnorm(1,  0, sigma_m)  
z_i <- function() 1 
````

The next scenario introduces growth uncertainty into the model, <span> \( x_{t+1} = f( z_m x_t) \) </span>, where `z_m` is lognormal with logmean 0 and logsd of `r sigma_m`. We solve the SDP this time using the simulation approach 
...
As before, we simulate 100 replicates using the same random number sequence, now under this case where the noise comes from measurement and is accounted for by the management, but not the growth noise.   
...
```{r }
sigma_m <- 0.15    # Noise in population growth
z_g <- function() rlnorm(1,  0, sigma_m)
z_m <- function() rlnorm(1,  0, sigma_m)  
z_i <- function() 1 
````
